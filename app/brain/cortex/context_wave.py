...
class ContextWave:
    ...
    def get_effective_timeout(self, default: float = 30.0) -> float:
        ...
        Loi de Moindre Action appliquée au temps.
        """
    ...
    def get_enriched_system(self) -> str:
        ...
        Loi de Résonance : zéro reconstruction.
        """
        if not self.memory:
            return self.query
    ...
    def next_wave(self, memory: tuple = ()) -> ContextWave:
        ...
        Pattern ChainWave pour multi-étapes.
        """
        return ContextWave(
            query=self.query,
    ...
    def create(query: str, budget: float = 15.0) -> ContextWave:
        """Point d'entrée unique — première onde. Thalamus détecte la fréquence."""
        try:
            from app.brain.synapses.thalamus import detect_all_frequencies
            frequencies = detect_all_frequencies(query)
            signals = {"frequencies": frequencies}
        except Exception:
